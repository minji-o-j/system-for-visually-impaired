...
def max_light(light):
    ...
    for i in range(0,len(light)):
        for x1, y1, x2, y2, conf, cls_conf, cls_pred in light[i]:
            ...
            if (min_x1 > x1 and abs(min_x1-x1)<50):
                min_x1 = x1
            if (max_x2 < x2 and abs(min_x1-x1)<50):
                max_x2 = x2
            if (min_y1 > y1 and abs(min_x1-x1)<50):
                min_y1 = y1
            if (max_y2 < y2 and abs(min_x1-x1)<50):
                max_y2 = y2
    ...
    for img_i, (path, detections) in enumerate(zip(imgs, img_detections)):
        ...
        if detections is not None:
            ...
            if len(light)>0: 
                ...
                if light_color(x1, y1, x2, y2,path)=="green" :
                    green+=1
                ...
                if green+red>24:
                    if green/24 > 0.7 and (move=='' or move=='wait'):
                        move = 'green_wait'
                        green=0
                        red=0
                    elif red/24 > 0.7 and (move=='' or move=='wait') :
                        move = 'stop'
                        green=0
                        red = 0
                    elif green/24 > 0.7 and move=='stop' :
                        move='go'
                        green=0
                        red = 0
                    else :
            ...
            if move=='green_wait':
                plt.text(
                    x1+50,
                    y1,
                    s='please wait next green light',  # classes[int(cls_pred)],
                    color="white",
                    verticalalignment="top",
                    bbox={"color": color, "pad": 0},
                )
                ...
                print('light is red please wait green light')
                plt.text(
                    x1+50,
                    y1,
                    s='light is red please wait green light',  # classes[int(cls_pred)],
                    color="white",
                    verticalalignment="top",
                    bbox={"color": color, "pad": 0},
                )
                ...
                plt.text(
                    x1+50,
                    y1,
                    s='gogogogogogogo',  # classes[int(cls_pred)],
                    color="white",
                    verticalalignment="top",
                    bbox={"color": color, "pad": 0},
                )

...
def max_light(detections):
    global max_x1, max_x2, max_y1, max_y2
    for x1, y1, x2, y2, conf, cls_conf, cls_pred in detections:
        if (cls_conf.item() > 0.98) and ('light' == classes[int(cls_pred)]):
            ...
            if (max_y2 < y2):
                max_y2 = y2
    return max_x1,max_x2,max_y1,max_y2

def max_crosswalk(detections):
    global max_x1, max_x2, max_y1, max_y2
    for x1, y1, x2, y2, conf, cls_conf, cls_pred in detections:
        if ('crosswalk' == classes[int(cls_pred)]):
            print("\t+ Label: %s, Conf: %.5f" % (classes[int(cls_pred)], cls_conf.item()))
            max_x1 = 0
            max_x2 = 0
            max_y1 = 0
...
if __name__ == "__main__":
    ...
    for img_i, (path, detections) in enumerate(zip(imgs, img_detections)):
        ...
        if detections is not None:
            ...
            x1, x2, y1, y2=max_light(detections)
            box_w = x2 - x1
            box_h = y2 - y1
            ...
            x1, x2, y1, y2 = max_crosswalk(detections)
            box_w = x2 - x1
            box_h = y2 - y1
